...
(function () {
  ...
  function initRevealingSection(element) {
    ...
    var observer = new IntersectionObserver(
      revealingSectionCallback.bind(element)
    );
  ...
  function animateRevealingSection() {
    if (this.prevElementTop - window.scrollY < window.innerHeight) {
      var opacity = (
        1 -
        (window.innerHeight + window.scrollY - this.prevElementTop) /
          window.innerHeight
      ).toFixed(2);
      if (opacity > 0) {
        this.resetOpacity = false;
  ...
  function updateOpacityValue(element, value) {
    element.element.style.setProperty(
      "--reavealing-section-overlay-opacity",
      value
    );
  }
  ...
  var revealingSection = document.getElementsByClassName(
